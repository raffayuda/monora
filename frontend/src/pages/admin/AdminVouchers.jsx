    ...
    <div className="p-6 md:p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1 m-0">{vouchers.length} voucher{vouchers.length !== 1 ? 's' : ''}</p>
        </div>
        <button onClick={openCreate} className="flex items-center gap-2 text-white px-5 py-2.5 rounded-xl text-sm font-semibold border-none cursor-pointer transition-all hover:opacity-90" style={{ background: 'linear-gradient(135deg, #f97316, #ea580c)' }}>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          { label: 'Total Vouchers', value: vouchers.length, color: '#f97316' },
          { label: 'Total Redeemed', value: vouchers.reduce((s, v) => s + v.usedCount, 0), color: '#10b981' },
          { label: 'Active Codes', value: vouchers.filter(v => v.usedCount < v.maxUses).length, color: '#3b82f6' },
        ].map(s => (
            ...
            {filtered.map((v, i) => (
              <motion.div key={v.code} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                ...
                <div className="col-span-1 flex items-center justify-end gap-1.5">
                  ...
                  <button onClick={() => setConfirmDelete(v.code)} className="w-8 h-8 rounded-lg flex items-center justify-center text-white/30 hover:text-red-400 hover:bg-red-500/10 transition-all border-none bg-transparent cursor-pointer" title="Delete">
